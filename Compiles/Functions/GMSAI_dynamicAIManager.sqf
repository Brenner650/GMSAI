			...
			if (isNil "_activePlayerGroup") then  // 
			{
				if (random(1) < GMSAI_dynamicRandomChance) then
				{
					...
					if (_dynamicAI isEqualTo []) then
					{
						...
						private _units = [GMSAI_dynamicRandomUnits] call GMSAI_getNumberFromRange;
						private _group = [_spawnPos,_units,GMSAI_alertAIDistance] call GMSAI_spawnInfantryGroup;	
						//_group setVariable["GMSAI_linkedPlayer",_player];
