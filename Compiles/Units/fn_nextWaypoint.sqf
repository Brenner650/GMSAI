...
if !(isNull _nearestEnemy) then
{
	...
	private _nextPos = [_groupPatrolArea,1,[_leader],35] call GMS_fnc_findRandomPosWithinArea select 0;	
	//diag_log format["_nextWaypoint: NO enemy neaarby | _groupPatrolArea = %1 | _nextPos = %2",_groupPatrolArea, _nextPos];	
	_group setVariable["timeStamp",diag_tickTime];
	private _wp = [_group, 0];
	_wp setWaypointPosition [_nextPos,5];
	_wp setWaypointSpeed (_group getVariable "GMSAI_waypointSpeed");
...
/*
		Logic: If enemies are neaby, approach a random position close to the nearest enemy else move to a random location within the area to be patrolled and loiter for a period (30 secs);
*/